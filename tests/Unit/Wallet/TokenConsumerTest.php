...
final class TokenConsumerTest extends TestCase
{
    ...
    public function testItDebitsThePriceTimesTheQuantityUnderTheGivenReference(): void
    {
        $this->walletOperator
            ->expects(self::once())
            ->method('debit')
        ...
        $this->createConsumer()->consume($this->customer, $this->createConsumable(5), 'cv-4521', 3);
    }

    public function testItRefusesAVariantWithoutATokenPrice(): void
    {
        $this->walletOperator->expects(self::never())->method('debit');

        $this->expectException(NotConsumableException::class);

        $this->createConsumer()->consume($this->customer, $this->createConsumable(null), 'cv-4521');
    }
    ...
    public function testItTellsWhetherTheBalanceCoversTheConsumption(): void
    {
        ...
        self::assertTrue($consumer->canConsume($this->customer, $this->createConsumable(5), 1));
        self::assertFalse($consumer->canConsume($this->customer, $this->createConsumable(5), 2));
    }
    ...
    public function testACustomerWithoutAWalletHasNothingToSpend(): void
    {
        ...
        self::assertFalse($this->createConsumer()->canConsume($this->customer, $this->createConsumable(1)));
    }
    ...
    private function createConsumable(?int $tokenPrice): TokenPackInterface
    {
        $variant = $this->createMock(TokenPackInterface::class);
        $variant->method('isConsumable')->willReturn(null !== $tokenPrice);
